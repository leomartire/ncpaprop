...
int NCPA::SolveModBB::computeModes() {	
  //
  ...
  int    NN, Nz_subgrid;
  double freq, admittance, dz_km, z_min_km, delZ, h2;	
  double k_min, k_max;			
  double *kreal, *kim;
  ...
  for (int ii = 0; ii<Nfreq; ii++) {
      ...
      i = getModalTrace(Nz_grid, z_min, sourceheight, receiverheight, dz, atm_profile, admittance, freq, azi, diag, &k_min, &k_max, 1); //WKB trick is turned off
      ...
      */
      ...
      if (out_dispersion) {
          ...
          writeDispersion_bb_ascii( disp_fn, select_modes, dz, \
...
int NCPA::SolveModBB::getModalTrace(\
...
int NCPA::SolveModBB::doSelect(int nz, int n_modes, double k_min, double k_max, double *k2, double **v, double *k_s, double **v_s, int *select_modes)
{
  int cnt = -1;
  int i, j;
  double k;
