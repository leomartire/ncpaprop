...
// constructor 2
//
NCPA::SolveCModNB::SolveCModNB(ProcessOptionsNB *oNB, SampledProfile *atm_profile)
{
...
NCPA::SolveCModNB::~SolveCModNB()
{
  ...
  delete [] rho;
  delete [] Pr;
  //printf("SolveCModNB destructor done.\n");
}
*/
...
void NCPA::SolveCModNB::printParams() {
  printf(" Complex Normal Modes run info:\n");
  printf("                   freq : %g\n", freq);
  if (!Nby2Dprop) {
  printf("                azimuth : %g\n", azi);
  }
...
int NCPA::SolveCModNB::computeModes() {
  //
  ...
  for (it=0; it<Naz; it++) {
    ...
    cout << "Processing azimuth = " << azi << endl;
    ...
    */
    ...
    if (Nby2Dprop) { // if (N by 2D is requested)
        getTLoss1DNx2(azi, select_modes, dz, Nrng_steps, rng_step, sourceheight, receiverheight, rho, k_s, v_s, Nby2Dprop, it);
    }
    else {
        cout << "Writing to file: 1D transmission loss at the ground..." << endl;
        getTLoss1D(select_modes,dz,Nrng_steps,rng_step,sourceheight, receiverheight, rho, k_s, v_s);

        if (write_2D_TLoss) {
		        cout << "Writing to file: 2D transmission loss...\n";
		        getTLoss2D(Nz_grid,select_modes,dz,Nrng_steps,rng_step,sourceheight,rho, k_s, v_s);
        }

        if (write_phase_speeds) {
	          cout << "Writing to file: phase speeds...\n";
	          writePhaseSpeeds(select_modes,freq,k_s);
        }
        ...
        if (write_dispersion) {
		        ...
		        //writeDispersion(select_modes,dz,sourceheight,receiverheight,freq,k_s, v_s);
		        writeDispersion(select_modes,dz,sourceheight,receiverheight,freq,k_s, v_s, rho);
        }
    }
...
int NCPA::SolveCModNB::getAbsorption(int n, double dz, SampledProfile *p, double freq, double *alpha)
{
...
int NCPA::SolveCModNB::getAbsorption(int n, double dz, SampledProfile *p, double freq, string usrattfile, double *alpha)
{
...
int NCPA::SolveCModNB::getCModalTrace(\
...
int NCPA::SolveCModNB::getNumberOfModes(int n, double dz, complex<double> *diag, double k_min, double k_max, int *nev)
{
  int nev_max, nev_min;
...
int NCPA::SolveCModNB::sturmCount(int n, double dz, complex<double> *diag, double k, int *cnt)
{
...
int NCPA::SolveCModNB::doSelect(int nz, int n_modes, double k_min, double k_max, complex<double> *k2, complex<double> **v, complex<double> *k_s, complex<double> **v_s, int *select_modes)
{
  int cnt = -1;
  int i, j;
  double k_r;
...
int NCPA::SolveCModNB::getTLoss1D(int select_modes, double dz, int n_r, double dr, double z_src, double z_rcv, double *rho, complex<double> *k_s, complex<double> **v_s)
{
  ...
  complex<double> modal_sum_i, modal_sum_i_ll; // for incoherent sum if desired
  ...
  FILE *tloss_1d    = fopen("tloss_1d.cnm","w");
  FILE *tloss_ll_1d = fopen("tloss_1d.lossless.cnm","w");
  ...
  for (i=0; i<n_r; i++) {
      ...
      // this version has the sqrtrho_ratio factor
      /*
      for (m=0; m<select_modes; m++) {
          modal_sum_c    = modal_sum_c + v_s[n_zsrc][m]*v_s[n_zrcv][m]*sqrtrho_ratio*exp(I*k_s[m]*r)/sqrt(k_s[m]);
          modal_sum_c_ll = modal_sum_c_ll + v_s[n_zsrc][m]*v_s[n_zrcv][m]*sqrtrho_ratio*exp(I*real(k_s[m])*r)/sqrt(real(k_s[m]));
          
          modal_sum_i    = modal_sum_i + pow(v_s[n_zsrc][m]*v_s[n_zrcv][m]*sqrtrho_ratio,2)*exp(-2*imag(k_s[m])*r)/abs(k_s[m]);
          modal_sum_i_ll = modal_sum_i_ll + pow(v_s[n_zsrc][m]*v_s[n_zrcv][m]*sqrtrho_ratio,2)/real(k_s[m]);
      }
      */
      ...
      for (m=0; m<select_modes; m++) {    
          modal_sum_c    = modal_sum_c    + v_s[n_zsrc][m]*v_s[n_zrcv][m]*exp(I*k_s[m]*r)/sqrt(k_s[m]);
          modal_sum_c_ll = modal_sum_c_ll + v_s[n_zsrc][m]*v_s[n_zrcv][m]*exp(I*real(k_s[m])*r)/sqrt(real(k_s[m]));
          modal_sum_i    = modal_sum_i    + pow(v_s[n_zsrc][m]*v_s[n_zrcv][m],2)*exp(-2*imag(k_s[m])*r)/abs(k_s[m]);
          modal_sum_i_ll = modal_sum_i_ll + pow(v_s[n_zsrc][m]*v_s[n_zrcv][m],2)/real(k_s[m]);
      } 
      ...
      fprintf(tloss_1d,"%f %20.12e %20.12e %20.12e %20.12e\n", r/1000.0, real(modal_sum_c), imag(modal_sum_c), real(modal_sum_i), imag(modal_sum_i));
      fprintf(tloss_ll_1d,"%f %20.12e %20.12e %20.12e %20.12e\n", r/1000.0, real(modal_sum_c_ll), imag(modal_sum_c_ll), real(modal_sum_i_ll), imag(modal_sum_i_ll));
  }
  fclose(tloss_1d);
  fclose(tloss_ll_1d);
  printf("           file tloss_1d.cnm created\n");
  printf("           file tloss_1d.lossless.cnm created\n");
  return 0;
}


int NCPA::SolveCModNB::getTLoss1DNx2(double azimuth, int select_modes, double dz, int n_r, double dr, double z_src, double z_rcv, double *rho, complex<double> *k_s, complex<double> **v_s, bool Nx2, int iter)
{
  // computes the transmissison loss from source to receiver.
  // The formula for pressure is:
  // p(r,z) = sqrt(rho(z)/rho(zs))*I*exp(-I*pi/4)/sqrt(8*pi*r)*sum(Vm(z)*Vm(zs)*exp(I*k_m*r)/sqrt(k_m))
  ...
  int n_zrcv = (int) ceil(z_rcv/dz);
  double r, sqrtrho_ratio;
  complex<double> modal_sum_i, modal_sum_i_ll;
  complex<double> modal_sum_c, modal_sum_c_ll;
  complex<double> I (0.0, 1.0);
  complex<double> expov8pi =  4*Pi*I*exp(-I*Pi*0.25)/sqrt(8.0*Pi); // the 4*Pi factor ensures that the modal sum below ends up being the actual TL
  ...
  if (iter==0) {
      tloss_1d    = fopen("Nby2D_tloss_1d.cnm","w");
      tloss_ll_1d = fopen("Nby2D_tloss_1d.lossless.cnm","w");
  }
  else {  // append
      tloss_1d    = fopen("Nby2D_tloss_1d.cnm","a");
      tloss_ll_1d = fopen("Nby2D_tloss_1d.lossless.cnm","a");
  }
  ...
  for (i=0; i<n_r; i++) {
      ...
      */
      ...
      fprintf(tloss_1d,"%10.3f %8.3f %20.12e %20.12e %20.12e %20.12e\n", r/1000.0, azimuth, real(modal_sum_c), imag(modal_sum_c), real(modal_sum_i), imag(modal_sum_i));
      fprintf(tloss_ll_1d,"%10.3f %8.3f %20.12e %20.12e %20.12e %20.12e\n", r/1000.0, azimuth, real(modal_sum_c_ll), imag(modal_sum_c_ll), real(modal_sum_i_ll), imag(modal_sum_i_ll));
  }
  fprintf(tloss_1d, "\n");
  fprintf(tloss_ll_1d, "\n");
...
/*
int NCPA::SolveCModNB::getTLoss2D(int nz, int select_modes, double dz, int n_r, double dr, double z_src, double *rho, complex<double> *k_s, complex<double> **v_s)
{
  ...
  FILE *tloss_2d = fopen("tloss_2d.cnm","w");
  ...
  for (i=0; i<n_r; i++) {
      ...
      for (j=0; j<nz; j=j+stepj) {
          ...
          //// Use the commented lines if the modes must be scaled by srt(rho)
          //for (m=0; m<select_modes; m++) {
          //    modal_sum = modal_sum + v_s[n_zsrc][m]*v_s[j][m]*sqrtrhoj*exp(I*k_s[m]*r)/sqrt(k_s[m]);
          //}
          //modal_sum = expov8pi*modal_sum/sqrt(r*rho_atzsrc));
                   
          for (m=0; m<select_modes; m++) {
              modal_sum = modal_sum + v_s[n_zsrc][m]*v_s[j][m]*exp(I*k_s[m]*r)/sqrt(k_s[m]);
          }
  ...
  fclose(tloss_2d);
  printf("           file tloss_2d.cnm created\n");
  return 0;
}
*/
...
int NCPA::SolveCModNB::writeDispersion(int select_modes, double dz, double z_src, double z_rcv, double freq, complex<double> *k_s, complex<double> **v_s)
{
  ...
  for(i=0;i<select_modes;i++) {
      fprintf(dispersion,"   %.12e   %.12e",real(k_s[i]),imag(k_s[i]));
...
int NCPA::SolveCModNB::writePhaseSpeeds(int select_modes, double freq, complex<double> *k_s)
{
  int j;
  FILE *phasespeeds= fopen("phasespeeds.cnm", "w");
  for (j=0; j< select_modes; j++) {
      fprintf(phasespeeds, "%d %f %15.8e\n", j, (2*Pi*freq)/real(k_s[j]), imag(k_s[j]));
  }
  fclose(phasespeeds);
...
int NCPA::SolveCModNB::writeEigenFunctions(int nz, int select_modes, double dz, complex<double> **v_s)
{
  char mode_output[40];
  int i,j;
  complex<double> chk;
...
int NCPA::SolveCModNB::doNormalize(int nz, int n_modes, double dz, complex<double> **v_s) {
  int i,j;
